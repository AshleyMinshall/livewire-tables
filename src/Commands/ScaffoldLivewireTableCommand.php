...
class ScaffoldLivewireTableCommand extends FileManipulationCommand
{
    ...
    protected function viewContents()
    {
        ...
        $className = $this->parser->baseClassNamespace . '\\' . $this->parser->className();

        $tableComponent = new $className(1);
        $css = $this->setCssArray($tableComponent->css);

        $fields = $tableComponent->fields();
        ...
        return preg_replace(
            '/\[header\]/',
            $this->headerRows($fields, $css),
            preg_replace(
                '/\[data\]/',
                $this->dataRows($fields, $css),
    ...
    protected function headerRows(array $fields, array $css)
    {
        ...
        foreach ($fields as $key => $field) {
            $cell = "\t\t\t<th" . $this->getHeaderClass($field, $css) . $this->getSortableAction(
                $key,
                $field
            ) . '>' . $field['title'] . '</th>';
            if (end($fields) !== $field) {
